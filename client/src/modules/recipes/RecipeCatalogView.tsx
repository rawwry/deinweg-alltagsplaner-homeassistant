    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-24 md:pb-8">
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        {isStaff && (
          <button
            ...
            className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-semibold flex items-center gap-1.5 shadow-sm transition-all self-stretch sm:self-auto justify-center"
          >
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200">
          ...
          <h3 className="text-base font-bold text-slate-700">Keine Rezepte gefunden</h3>
          <p className="text-xs text-slate-500 mt-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((recipe) => (
            <div
              ...
              className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-300 cursor-pointer transition-all flex flex-col justify-between group"
            >
              <div>
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                ...
                <button
                  type="button"
                  className="px-3 py-1.5 bg-slate-50 group-hover:bg-indigo-600 group-hover:text-white text-slate-700 rounded-xl font-semibold transition-colors flex items-center gap-1 text-xs"
                >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-100 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-800">Neues Rezept anlegen</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateRecipe} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Titel des Gerichts</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Zeit (Min)</label>
                  <input
                    type="number"
                    value={newPrepTime}
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Basis-Port.</label>
                  <input
                    type="number"
                    value={newServings}
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Kurzbeschreibung</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Zubereitungsschritte</label>
                <textarea
                  rows={4}
